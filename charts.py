...
def _fig(fig, title="", h=None):
    fig.update_layout(**LAYOUT, title=dict(text=title, font=dict(size=12, color=NAVY2, weight="bold")))
...
def status_bar(issues):
    ...
    fig = go.Figure(go.Bar(
        ...
        textfont=dict(color=NAVY2, size=11, weight="bold"),
    ))
    fig.update_layout(**LAYOUT, title=dict(text="Status Distribution", font=dict(size=12,color=NAVY2,weight="bold")),
...
def assignee_stacked(issues, labels):
    ...
    fig.update_layout(**LAYOUT, barmode="stack",
                      title=dict(text="Assignee Load by Label", font=dict(size=12,color=NAVY2,weight="bold")),
...
def bubble_chart(issues):
    ...
    fig.update_layout(**LAYOUT,
                      title=dict(text="Load vs Staleness (bubble = overdue count)", font=dict(size=12,color=NAVY2,weight="bold")),
...
def heatmap(issues):
    ...
    fig.update_layout(**LAYOUT,
                      title=dict(text="Assignee × Status Heatmap", font=dict(size=12,color=NAVY2,weight="bold")),
...
def priority_donut(issues):
    ...
    fig.update_layout(**LAYOUT, title=dict(text="Priority (Open)", font=dict(size=12,color=NAVY2,weight="bold")))
    return fig
...
def type_donut(issues):
    ...
    fig.update_layout(**LAYOUT, title=dict(text="Issue Type Split", font=dict(size=12,color=NAVY2,weight="bold")))
    return fig
...
def age_histogram(issues):
    ...
    fig.update_layout(**LAYOUT,
                      title=dict(text="Issue Age (days open)", font=dict(size=12,color=NAVY2,weight="bold")),
