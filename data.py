...
PROJECT = os.getenv("JIRA_PROJECT", "NNG")
...
def _fetch_all():
    issues, start = [], 0
    while True:
        res = _get("/search", {"jql": f"project={PROJECT} ORDER BY updated DESC",
...
def get_assignees(issues):
    return sorted(set(i["assignee"] for i in issues))
