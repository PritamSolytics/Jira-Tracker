...
def monte_carlo_sprint(issues, n_sim=1000):
    ...
    for _ in range(n_sim):
        ...
        for issue in open_issues:
            ...
            remaining = max(0, issue.get("days_stale",0))
...
def predicted_slips(issues):
    ...
    for i in open_i:
        risk = 0
        if "Past Due" in i.get("due_flag",""): risk += 50
...
def layout(issues):
    ...
    past_due = [i for i in open_issues if "Past Due" in i.get("due_flag","")]
    ...
    # ── Recommended actions ────────────────────────────────────────────────────
    actions = _generate_actions(issues, conf, past_due, blockers, unassigned, slips)
    ...
    slip_card = C.card(
        ...
        html.Table([
            html.Thead(html.Tr([html.Th(h) for h in
                                ["Issue","Assignee","Summary","Status","Due","Slip Risk"]],
...
def _generate_actions(issues, conf, past_due, blockers, unassigned, slips):
    actions = []
    ...
    # Overdue action
    if past_due:
        top_overdue = sorted(past_due, key=lambda x: x.get("days_stale",0), reverse=True)[:2]
        names = ", ".join(i["key"] for i in top_overdue)
        actions.append({
            "action": f"Escalate {len(past_due)} overdue issues — prioritize {names}",
